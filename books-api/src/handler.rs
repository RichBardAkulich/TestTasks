...
#[get("/books-api")]
async fn books_checker_handler() -> impl Responder {
...
async fn edit_todo_handler(
    ...
    let author = body.author.to_owned().unwrap_or(todo.author.to_owned());
    let year = body.year.to_owned().unwrap_or(todo.year.to_owned());
    let payload = Todo {
        id: todo.id.to_owned(),
